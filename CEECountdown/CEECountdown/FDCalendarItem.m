...
- (NSString *)chineseCalendarOfDate:(NSDate *)date {
    ...
    if ((currentYear == 2017) && (currentMonth == 6)) {
        if (currentDay == 7 || currentDay == 8 || currentDay == 9) {
            return @"高考";
        }
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.dayLabel.textColor = [UIColor colorWithRed:106.f/255.f green:57.f/255.f blue:6.f /255.f alpha:1];
    cell.chineseDayLabel.textColor = [UIColor colorWithRed:106.f/255.f green:57.f/255.f blue:6.f /255.f alpha:1];
    ...
    if (indexPath.row < firstWeekday) {    // 小于这个月的第一天
        ...
        NSInteger dayday=[components day];
        
        if ((day == dayday) && [[NSCalendar currentCalendar] isDate:[NSDate date] equalToDate:self.date toUnitGranularity:NSCalendarUnitMonth]) {
            cell.backgroundColor = [UIColor colorWithRed:255.f/255.f green:246.f/255.f blue:103.f/255.f alpha:1];
        ...
        if ([[[UIDevice currentDevice]systemVersion]floatValue]>=8.0) {
            if ([[NSCalendar currentCalendar] isDate:[NSDate date] equalToDate:self.date toUnitGranularity:NSCalendarUnitMonth] && ![[NSCalendar currentCalendar] isDateInToday:self.date]) {
                NSDateComponents *components1=[[NSCalendar currentCalendar]components:NSCalendarUnitDay fromDate:[NSDate date]];
                NSInteger dayday1=[components1 day];
                //[[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:[NSDate date]]
                // 将当前日期的那天高亮显示
                if (day == dayday1) {
                    cell.dayLabel.textColor = [UIColor colorWithRed:233.f/255.f green:164.f/255.f blue:0.f/255.f alpha:1];
                    cell.chineseDayLabel.textColor = [UIColor colorWithRed:233.f/255.f green:164.f/255.f blue:0.f/255.f alpha:1];
                }
            }
