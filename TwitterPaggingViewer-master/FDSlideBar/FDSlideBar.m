...
- (instancetype)init {
    CGRect frame = CGRectMake(0, 23, DEVICE_WIDTH, 46);
...
- (void)setupItems {
    ...
    for (NSString *title in _itemsTitle) {
        ...
        itemX = CGRectGetMaxX(item.frame)+ITEM_PADDING;
    }
    ...
    _scrollView.center = CGPointMake(self.frame.size.width/2.0f, _scrollView.center.y);
    ...
    _sliderView.frame = CGRectMake(0, (self.frame.size.height - SLIDER_VIEW_HEIGHT)/2.0f, firstItem.frame.size.width, SLIDER_VIEW_HEIGHT);
    _sliderView.layer.cornerRadius = CGRectGetHeight(_sliderView.frame)/2.0f;
